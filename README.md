...
### 이번 주 한눈에 보기
- 월~일 7일을 같은 규칙(학기/방학·공휴일·하루 예외·요일별 시간표)으로 평가해 막대 그래프로 요약
- 요일별 교시 수와 하루 범위, 남은 수업일, 주간 수업 시간 합계, 가장 빡센 날 강조
...
- 다크/라이트/시스템 테마(첫 페인트 전 부트스트랩으로 FOUC 없음), 키보드 단축키(`S` 설정 · `T` 화면 모드 · `F` 전체 화면 · `M` 소리 · `N` 알림) — 모두 실제로 동작
